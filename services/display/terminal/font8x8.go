...
// font8x8.go — public-domain-style 8×8 bitmap font covering printable
// ASCII (0x20..0x7E). One byte per row, top to bottom; bit 7 = leftmost
// pixel. Indexed by [ch - 32].
...
// Glyph returns the 8-row bitmap for ch (unprintable → space).
func Glyph(ch byte) [8]byte {
	if ch < 32 || ch > 126 {
		return font8x8[0]
	}
